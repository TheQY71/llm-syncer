document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!sendBtn || !promptInput || !statusDiv || !autoSendCheckbox || !llmListDiv) {
    console.error("[MultiLLM][popup] 必要的 DOM 元素缺失，请检查 popup.html 的 id。");
    return;
  }
  ...
  function init() {
    chrome.tabs.query({ currentWindow: true }, (tabs) => {
      ...
      console.log("[MultiLLM][popup] detected tabs:", detectedTabs.map((t) => t.url));
      renderLLMList();
    });
  }
  ...
  sendBtn.addEventListener("click", () => {
    ...
    const targetTabs = detectedTabs.filter((tab) => selectedTabIds.includes(tab.id));
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            ...
            if (!msg.includes("Receiving end does not exist")) {
              console.warn("[MultiLLM][popup] Error sending to tab", tab.id, msg);
            }
